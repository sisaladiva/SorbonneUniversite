...
{
	double start_total = wallclock_time();
	MPI_Init(NULL, NULL);
	int rank, size;
...
unsigned char *image = malloc(w * h);
	...
	int lignes_par_proc = h/size;
	int debut = rank * lignes_par_proc;
	int fin = (rank == size - 1) ? h : debut + lignes_par_proc;
	unsigned char *local_image = malloc(w * (fin - debut));

	/* Processing the grid point by point */
	double y = ymin + debut * yinc;
	for (int i = debut; i < fin ; i++) {
		double x = xmin;
		for (int j = 0; j < w; j++) {
			local_image[j + (i-debut) * w] = xy2color(x, y, depth);
			x += xinc;
		}
		y += yinc;
	}
...
fprintf(stderr, "rank%d Total computing time: %g sec\n", rank, end - start);
	...
	MPI_Finalize();
	double end_tot = wallclock_time();
		fprintf(stderr, " TOOOTALL: %g sec\n", end_tot - start_total);
