...
int golden_claw_search_mpi(int maxres, u64 **k1, u64 **k2, int rank, int num_processes) {
    double start = wtime();  // Start the timer for measuring execution time
    u64 N = 1ull << n;  // Set N to 2^n
    u64 local_dict_size = (1.125 * N) / (num_processes * 2);  // Adjust the local dictionary size for each process

    // Check if the local dictionary size is too small for the number of processes
    if (local_dict_size < 2) {
        fprintf(stderr, "Dictionary size is too small for the number of processes\n");
        MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);  // Abort if the size is insufficient
        return 0;
    }

    // Calculate the number of bits needed for masking based on the number of processes
    int number_bits = log2(num_processes);
    u64 mask = (1ULL << number_bits) - 1;

    char hdsize[8];
    human_format(local_dict_size * sizeof(*A), hdsize);  // Format the size of the dictionary
    printf("Process %d: Local dictionary size: %sB\n", rank, hdsize);  // Print the local dictionary size for the process
    ...
    *k2 = malloc(maxres * sizeof(u64));
    if (*k1 == NULL || *k2 == NULL) {
        ...
        MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);  // Abort if memory allocation fails
        return 0;
    }
    ...
    for (int parity = 0; parity < 2; parity++) {
        ...
        #pragma omp parallel 
        {
            ...
            #pragma omp for 
            for (u64 x = 0; x < N; x += 1) {
                u64 z = f(x);  // Apply function f to x

                // Check if the parity matches and the masked value matches the rank
               if (((z & 1) == (u64)parity) && ((x & mask) == (u64)rank)) {
                    if (local_count >= (local_dict_size / omp_get_num_threads())) {
                        fprintf(stderr, "Local entries array out of bounds\n");
                        free(local_entries);
                        MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);  // Abort if array bounds are exceeded
                    }
                    local_entries[local_count].k = z % PRIME;  // Store the key (mod PRIME)
                    local_entries[local_count].v = x;  // Store the value
                    local_count++;
                }
            }
            ...
            #pragma omp critical
            {
                // Insert the local entries into the dictionary
            ...
            free(local_entries);  // Free the local entries after processing
        }
        ...
        #pragma omp parallel reduction(+:ncandidates)
        {
            ...
            int local_nres = 0;
            memset(local_x, 0, sizeof(local_x));  // Initialize local_x array
            memset(local_k1, 0, sizeof(local_k1));  // Initialize local_k1 array
            memset(local_k2, 0, sizeof(local_k2));  // Initialize local_k2 array

            #pragma omp for schedule(dynamic, 1024)
            for (u64 z = 0; z < N; z++) {
                u64 y = g(z);  // Apply function g to z
                if (((y & 1) == (u64)parity)) {
                    int nx = dict_probe(y, 256, local_x);  // Probe the dictionary for matches
                    ncandidates += nx;

                    #pragma omp simd
                    for (int j = 0; j < nx; j++) {
                        if (is_good_pair(local_x[j], z)) {  // Check if the pair is valid
                            if (local_nres < 1024) {
                                local_k1[local_nres] = local_x[j];  // Store the valid key
                                local_k2[local_nres] = z;  // Store the valid value
                                local_nres++;
                            }
                        }
                    }
            ...
            {
                // Store the results in the global arrays k1 and k2
        ...
        dict_clear();  // Clear the dictionary for the next iteration
    }

    double end_time = wtime();  // Measure the end time
    printf("Process %d: Execution time: %.5f seconds\n", rank, end_time - start);  // Print the execution time for the process

    return local_nkey;  // Return the number of local keys found
}
