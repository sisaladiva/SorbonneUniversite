...
int golden_claw_search(int maxres, u64 k1[], u64 k2[])
{
   ...
   #pragma omp parallel
{
    struct entry *local_dict = malloc(sizeof(*A) * dict_size);
    if (!local_dict) err(1, "impossible to allocate local dictionnary");
    for (u64 i = 0; i < dict_size; i++) local_dict[i].k = EMPTY;
#pragma omp for schedule(static)
for (u64 x = 0; x < N; x++) {
    u64 z = f(x);
    u64 h = (murmur64(z) + omp_get_thread_num() * (dict_size / omp_get_num_threads())) % dict_size;
    while (A[h].k != EMPTY) {
        h = (h + 1) % dict_size;
    }
    #pragma omp critical
    {
        local_dict[h].k = z % PRIME;
        local_dict[h].v = x;
    }
}
    ...
    {
        for (u64 i = 0; i < dict_size; i++) {
            if (local_dict[i].k != EMPTY) dict_insert(local_dict[i].k, local_dict[i].v);
        }
    }
    free(local_dict);
}
