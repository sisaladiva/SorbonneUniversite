    ...
    *k2 = malloc(maxres * sizeof(u64));
    ...
    for (int parity = 0; parity < 2; parity++) {
        ...
        #pragma omp parallel 
        {
            ...
            #pragma omp for
            for (u64 x = rank; x < N; x += num_processes) {
                u64 z = f(x);

                // Check if the parity matches
                if ((z & 1) == parity) {

                    // Check for local entries array bounds
                    ...
                    // Store the key-value pair in local entries
            ...
            // Insert local entries into the global dictionary
            #pragma omp critical
            {
        ...
        // Initialize the number of candidates found
        u64 ncandidates = 0;


       #pragma omp parallel reduction(+:ncandidates)
{
    // Declare local arrays to store results
    u64 local_x[256]; // Array to store results from dict_probe
    u64 local_k1[1024], local_k2[1024]; // Arrays to store solution pairs
    int local_nres = 0; // Counter for the number of local results found
    ...
    for (u64 z = 0; z < N; z++) {
        // Apply the function g to z to obtain y
        u64 y = g(z);
        
        // Check if the parity of y matches the current parity
        if ((y & 1) == parity) {
            // Search for values associated with y in the dictionary
            int nx = dict_probe(y, 256, local_x);
            ncandidates += nx; // Add the number of candidates found to the shared variable

            // Loop to process the found results
            for (int j = 0; j < nx; j++) {
                // Check if the pair (local_x[j], z) is valid
                if (is_good_pair(local_x[j], z)) {
                    // Ensure we do not exceed the maximum size of the local array
                    if (local_nres < 1024) {
                        // Store the found pair in the local arrays
                        local_k1[local_nres] = local_x[j];
                        local_k2[local_nres] = z;
                        local_nres++; // Increment the local result counter
                    }
                }
            }
        }
    ...
    // Critical section to avoid access conflicts when writing to global arrays
    #pragma omp critical
    {
        // Copy local results into the global arrays k1 and k2
        for (int i = 0; i < local_nres && local_nkey < maxres; i++) {
            (*k1)[local_nkey] = local_k1[i]; // Add to k1
            (*k2)[local_nkey] = local_k2[i]; // Add to k2
            local_nkey++; // Increment the global result counter
        }
    }
}
        dict_clear();
...
int main(int argc, char **argv)
{
    ...
    k1 = malloc(16 * sizeof(u64));  
    k2 = malloc(16 * sizeof(u64)); 
    if (k1 == NULL || k2 == NULL) {
