...
printf(" La taille du dictionnaire local est %d\n", local_dict_size);
   ...
   // Chaque processus remplit sa propre table de hachage et effectue une recherche avant de libérer la table
for (int parity = 0; parity < 2; parity++) { // Parity loop: 0 for even, 1 for odd
    dict_setup(local_dict_size); // Crée une nouvelle table de hachage pour cette parité

    // Remplissage de la table de hachage
    #pragma omp parallel for
    for (u64 x = 0; x < N; x++) {
        u64 z = f(x); // Calculer f(x)

        // Vérifier si f(x) correspond à la parité actuelle
        if ((z % 2) == parity) {
            int number_bits = log2(num_processes);
            u64 mask = (1ULL << number_bits) - 1;
            int target_process = (x & mask); // Déterminer le processus cible

            if (target_process == rank) { // Vérifier si ce processus est responsable de x
                #pragma omp critical
                dict_insert(z, x); // Insérer dans la table de hachage
            }
        }
    }

    // Recherche dans la table de hachage pour cette parité
    u64 ncandidates = 0;
    u64 x[256]; // Tableau temporaire pour stocker les correspondances
    #pragma omp parallel for reduction(+:ncandidates)
    for (u64 z = 0; z < N; z++) {
        u64 y = g(z); // Calculer g(z)

        // Recherche locale dans la table pour la valeur y
        int nx = dict_probe_local(y, 256, x, rank, num_processes);
        assert(nx >= 0); // Vérifie qu'aucune erreur ne s'est produite
        ncandidates += nx; // Compte le nombre de correspondances

        // Vérification des paires et stockage des résultats
        for (int i = 0; i < nx; i++) {
            if (is_good_pair(x[i], z)) { // Vérifie si (x[i], z) est une bonne paire
                #pragma omp critical
                {
                    if (nres == maxres) { // Si le nombre maximum de solutions est atteint
                        return -1; // Arrête la recherche
                    }
                    k1[nres] = x[i];
                    k2[nres] = z;
                    printf("SOLUTION FOUND! (process %d)\n", rank);
                    nres += 1; // Incrémente le nombre de solutions trouvées
                }
            }
        }
    }

    // Afficher un message pour chaque parité
    if (parity == 0) {
        printf("Process %d: Hash table filled and searched with even f(x).\n", rank);
    } else {
        printf("Process %d: Hash table filled and searched with odd f(x).\n", rank);
    }

    dict_clear(); // Libérer la table de hachage pour la parité actuelle
}
