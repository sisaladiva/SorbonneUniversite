...
int golden_claw_search_mpi(int maxres, u64 k1[], u64 k2[], int rank, int num_processes)
{
    ...
    for (int parity = 0; parity < 2; parity++) {
        ...
        #pragma omp parallel
{
    struct entry *local_table;
    dict_setup_local(local_dict_size / omp_get_num_threads(), &local_table);
    ...
    for (u64 x = 0; x < N; x++) {
        ...
        if ((z % 2) == parity) {
            int number_bits = log2(num_processes);
            u64 mask = (1ULL << number_bits) - 1;
            int target_process = (x & mask);
            if (target_process == rank) {
                u64 h = murmur64(z) % (local_dict_size / omp_get_num_threads());
                while (local_table[h].k != EMPTY) {
                    h = (h + 1) % (local_dict_size / omp_get_num_threads());
                }
                local_table[h].k = z % PRIME;
                local_table[h].v = x;
            }
        }
    }

    // Fusionner les sous-tables locales dans la table globale
    #pragma omp critical
    {
        for (u64 i = 0; i < local_dict_size / omp_get_num_threads(); i++) {
            if (local_table[i].k != EMPTY) {
                dict_insert(local_table[i].k, local_table[i].v);
            }
        }
    }
    ...
    free(local_table);
        ...
        for (u64 z = 0; z < N; z += 4) {
            // Calculer g(z) pour deux valeurs consécutives
            cache.z[0] = z;
            cache.z[1] = z + 1;
            cache.z[2] = z + 2;
            cache.z[3] = z + 3;
            ...
            cache.gz[2] = g(z + 2);
            cache.gz[3] = g(z + 3);
// Traiter les deux valeurs
int solutions_found = 0;  // Nombre de solutions trouvées
for (int i = 0; i < 4; i++) {
    u64 y = cache.gz[i];
    if ((y % 2) == parity) {
        ...
        for (int k = 0; k < nx; k++) {
            if (is_good_pair(x[k], cache.z[i])) {
                if (solutions_found < maxres) {
                    k1[nres] = x[k];
                    k2[nres] = cache.z[i];
...
// Après la boucle, vous pouvez gérer le cas où maxres a été atteint
if (solutions_found == maxres) {
    printf("Maximum number of solutions reached.\n");
    // Vous pouvez décider de continuer ou de sortir proprement
}
