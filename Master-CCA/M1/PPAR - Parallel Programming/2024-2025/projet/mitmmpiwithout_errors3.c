...
{
    u64 local_x[256]; // Tableau local pour éviter les conflits de threads
    u64 local_k1[1024], local_k2[1024]; // Solutions locales
    u64 local_nres = 0;
    ...
    for (u64 z = 0; z < N; z += 4) {
        ...
        // Vectorisation avec SIMD pour calculer g(z) en parallèle
        #pragma omp simd
        for (int i = 0; i < 4; i++) {
            y[i] = g(z + i);
        }
        ...
        for (int i = 0; i < 4; i++) {
            if ((y[i] & 1) == parity) { // Remplacer modulo par AND pour optimiser
                __builtin_prefetch(&x[0], 0, 3); // Préchargement des données pour dict_probe
                int nx = dict_probe(y[i], 256, local_x);
                ncandidates += nx;

                for (int j = 0; j < nx; j++) {
                    if (is_good_pair(local_x[j], z + i)) {
                        local_k1[local_nres] = local_x[j];
                        local_k2[local_nres] = z + i;
                        local_nres++;
                    }
                }
            }
    ...
    // Section critique pour fusionner les résultats locaux
    #pragma omp critical
    {
        for (u64 i = 0; i < local_nres; i++) {
            if (nres < maxres) {
                k1[nres] = local_k1[i];
                k2[nres] = local_k2[i];
                nres++;
