...
int main(int argc, char *argv[])
{
	...
	int rang;		// Initialisation de MPI 
	int p;
	MPI_Status status;
	MPI_Init(&argc,&argv);
...
// Calcul des subdivisions pour chaque axe
int sub_n = n / p; 
int sub_m = m / p; 
int sub_o = o / p; 

// Calcul de l'indice de début de la section pour ce processus
int start_n = sub_n * rang; 
int start_m = sub_m * rang; 
int start_o = sub_o * rang; 

// Calcul de l'indice de fin de la section pour ce processus
int end_n = (rang == p - 1) ? n : start_n + sub_n; 
int end_m = (rang == p - 1) ? m : start_m + sub_m;
int end_o = (rang == p - 1) ? o : start_o + sub_o; 

// Parcourir la section spécifique pour ce processus
for (int k = start_o; k < end_o; k++) {
    for (int i = start_n; i < end_n; i++) {
        for (int j = start_m; j < end_m; j++) {
            // Calcul de l'indice 1D correspondant aux indices 3D (i, j, k)
            int u = i + j * n + k * n * m;

            // Opérations sur les éléments du tableau R et T
...
for (int k = rang * (o / p); k < (rang + 1) * (o / p); k++) {
    ...
    // echange de données de haut en bas
    if (rang < p - 1) {
        MPI_Sendrecv(&T[((k + 1) * n * m) - n * m], n * m, MPI_DOUBLE, rang + 1, 0,
                     &T[(k + 1) * n * m], n * m, MPI_DOUBLE, rang + 1, 0, MPI_COMM_WORLD, &status);
    }

    if (rang > 0) {
        MPI_Sendrecv(&T[k * n * m], n * m, MPI_DOUBLE, rang - 1, 0,
                     &T[(k * n * m) - n * m], n * m, MPI_DOUBLE, rang - 1, 0, MPI_COMM_WORLD, &status);
    }

    // de gauche à droite
    if (rang % (p / 2) != 0) {
        MPI_Sendrecv(&T[k * n * m], n * (m / 2), MPI_DOUBLE, rang - 1, 0,
                     &T[k * n * m - 1], n * (m / 2), MPI_DOUBLE, rang - 1, 0, MPI_COMM_WORLD, &status);
    }

    if ((rang + 1) % (p / 2) != 0) {
        MPI_Sendrecv(&T[(k + 1) * n * m - 1], n * (m / 2), MPI_DOUBLE, rang + 1, 0,
                     &T[(k + 1) * n * m], n * (m / 2), MPI_DOUBLE, rang + 1, 0, MPI_COMM_WORLD, &status);
    }
}
// Calcul de la convergence et des informations agrégées sur la température maximale
if (n_steps % ((int)(1 / dt)) == 0) {
    double delta_T = 0;
    double max = -INFINITY;

    // Calcul de la convergence pour la partie attribuée à ce processus
    ...
    // Réduction MPI pour obtenir la somme de delta_T sur tous les processus
