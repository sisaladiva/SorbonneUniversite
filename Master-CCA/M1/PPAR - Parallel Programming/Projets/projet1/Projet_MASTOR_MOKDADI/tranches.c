...
printf("%d %d\n",o*n*m,n*m*(rang+1)*o/p );
    ...
    while (convergence == 0) {
			...
			if (rang<p-1) //echange entre le processus courant et le suivant
			MPI_Sendrecv(&T[((rang+1)*(o/p)*n*m)-n*m],n*m, MPI_DOUBLE,rang+1,0,&T[(rang+1)*(o/p)*n*m], n*m, MPI_DOUBLE,rang+1, 0,MPI_COMM_WORLD, &status);
			
			if (rang > 0) //puis echange entre le processus courant et le précédent
			MPI_Sendrecv(&T[rang*(o/p)*n*m],n*m, MPI_DOUBLE,rang-1,0,&T[((rang)*(o/p)*n*m)-n*m], n*m, MPI_DOUBLE,rang-1, 0,MPI_COMM_WORLD, &status);
